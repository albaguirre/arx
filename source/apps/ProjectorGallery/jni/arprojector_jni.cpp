 ...
 */
static void setContext(JNIEnv *env, jobject thiz, ARProjectorJniContext *value)
{
 ...
 */
static ARProjectorJniContext *getContext(JNIEnv *env, jobject thiz)
{
    jclass clazz = env->GetObjectClass(thiz);
    jfieldID field = env->GetFieldID(clazz, "context", "I");
    return reinterpret_cast<ARProjectorJniContext *>(env->GetIntField(thiz, field));
}
 ...
 */
static jboolean create(JNIEnv* env, jobject thiz, jfloatArray viewMatrix)
{
    ARProjectorJniContext *context = new ARProjectorJniContext(env, thiz);
    if (context != NULL) {
        if (context->setup(viewMatrix)) {
...
{
    ARProjectorJniContext *context = getContext(env, thiz);
    if (context != NULL) {
        setContext(env, thiz, NULL);
        delete context;
    }
...
static void zoomChanged(JNIEnv* env, jobject thiz, jfloat value)
{
    ARProjectorJniContext *context = getContext(env, thiz);
    if (context != NULL) {
        context->zoomChanged(value);
    }
}

static const JNINativeMethod g_methods[] = {
    { "create",          "([F)Z",                     (void*)create },
    { "destroy",         "()V",                         (void*)destroy },
    { "reset",           "()V",                         (void*)reset },
    { "zoomChanged",     "(F)V",                        (void*)zoomChanged }
};
...
extern "C" jint JNI_OnLoad(JavaVM* vm, void* reserved __attribute__((unused)))
{
    ...
    if (jniRegisterNativeMethods(env, "com/ti/arx/ProjectorGallery/ProjectorPoseRenderer", g_methods, NELEM(g_methods)) < 0) {
