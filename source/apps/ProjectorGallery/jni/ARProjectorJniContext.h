...
class ARProjectorJniContext :
    public ARXPropertyListener,
    public ARXFlatBufferListener
{
public:
    ARProjectorJniContext(JNIEnv *env, jobject obj);
    ~ARProjectorJniContext();

    bool setup(jfloatArray viewMatrix);
    void zoomChanged(jfloat zoom);
    void reset();
